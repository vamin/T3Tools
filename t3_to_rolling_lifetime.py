...
dtime_ns = dtime[chan==1] * Resolution

# create bins
binsize = 1 # ns
binmax = max(dtime_ns) # ns
nbinmax = int( math.ceil(binmax / binsize) ) + 1

bins = [0]
for i in xrange (1, nbinmax):
  bins.append(i*binsize)

# step through data in chunks, compute lifetime and print
chunk = 1e3
range_end = int(len(dtime_ns)/chunk) + 1
print "range end: %r" % range_end

for i in xrange (1, range_end):
  slice_start = (i-1) * chunk
  slice_end = i * chunk
  (counts, times) = np.histogram(dtime_ns[slice_start:slice_end], bins)
  y = np.log(counts)
  tau, a_log = np.polyfit(times[0:-1], y, 1)
  a = np.exp(a_log)
  t = -1/tau
  print "%r" % t
  print "%r" % a
